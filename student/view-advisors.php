...
if (isset($_POST['load_data'])) {
    ...
    $sqlStudentMinor = "SELECT a.*, concat(u.first_name, ' ', u.last_name) as advisor_name";
    $sqlStudentMinor .= " FROM `advisor` as a";
    $sqlStudentMinor .= " LEFT JOIN `student` as s ON s.id = a.student_id";
    $sqlStudentMinor .= " LEFT JOIN `faculty` as f ON f.id = a.faculty_id";
    $sqlStudentMinor .= " LEFT JOIN `users` as u ON u.id = f.user_id";

    $sqlStudentMinor .= " WHERE s.user_id = '$user_id'";

    if(!empty($search_text)) {
        $sqlStudentMinor .= " AND (a.time_of_advisement LIKE '%$search_text%'";
        $sqlStudentMinor .= " OR u.first_name LIKE '%$search_text%' OR u.last_name LIKE '%$search_text%')";
    }

    $totalQuery = mysqli_query($conn, $sqlStudentMinor);
    $total_count = mysqli_num_rows($totalQuery);

    $sqlStudentMinor .= " LIMIT $page_size OFFSET $start_number";


    $query = mysqli_query($conn, $sqlStudentMinor);
...
//        var_dump($row["prereq_course_name"]);
        ...
        $resultHtml .= '<td>'. $row["advisor_name"].'</td>';
...
<script src="../js/student/view-advisors.js"></script>
