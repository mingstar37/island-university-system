...
if (isset($_POST['load_data'])) {
    ...
    $sqlStudentMinor = "SELECT sm.*, m.minor_name, d.name as department_name";
    $sqlStudentMinor .= " FROM `student_minor` as sm";
    $sqlStudentMinor .= " LEFT JOIN `student` as s ON s.id = sm.student_id";
    $sqlStudentMinor .= " LEFT JOIN `minor` as m ON m.id = sm.minor_id";
    $sqlStudentMinor .= " LEFT JOIN `department` as d ON d.id = m.department_id";

    $sqlStudentMinor .= " WHERE s.user_id = '$user_id'";

    if(!empty($search_text)) {
        $sqlStudentMinor .= " AND (sm.type LIKE '%$search_text%'";
        $sqlStudentMinor .= " OR m.minor_name LIKE '%$search_text%' OR d.name LIKE '%$search_text%')";
    }
...
<script src="../js/student/view-minors.js"></script>
