...
if (isset($_POST['load_data'])) {
    ...
    $sqlHold = "SELECT sh.*";
    $sqlHold .= " FROM `student_holds` as sh";
    $sqlHold .= " INNER JOIN `student` as s ON s.id = sh.student_id";

    $sqlHold .= " WHERE s.user_id = '$user_id'";

    if(!empty($search_text)) {
        $sqlHold .= " AND (sh.id LIKE '%$search_text%'";
        $sqlHold .= " OR sh.hold_date LIKE '%$search_text%' OR sh.hold_type LIKE '%$search_text%')";
    }

    $totalQuery = mysqli_query($conn, $sqlHold);
    $total_count = mysqli_num_rows($totalQuery);

    $sqlHold .= " LIMIT $page_size OFFSET $start_number";



    $query = mysqli_query($conn, $sqlHold);
...
<script src="../js/student/view-holds.js"></script>
