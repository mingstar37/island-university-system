...
function onLoadInitSelector() {
    ...
    for (let i = curYear + 3; i > curYear - 15; i--) {
...
function onUpdateStatus(event, id) {
    event.preventDefault();
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res != undefined) {
                ...
                let year  = res.year ;
                $('.year-selectpicker').selectpicker('val', year );

                let term = res.term;
                $('.term-selectpicker').selectpicker('val', term);
